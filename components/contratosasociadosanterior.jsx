  ...
  const cargarInformex = async (elinforme) => {
    ...
    //if (deuda === 0) {
      ...
      if (datax1.length > 0) {
        ...
        settamortizadosalod(datax1[0].anticipo - datax1[0].amortizado);
        setFechainforme(datax1[0].fecha)
        setValue1(formatNumber(datax1[0].valor))
        setValue2(formatNumber(datax1[0].amortizar))
        console.log('Valor set:'+value1)
        setDate({
          start: parseAbsoluteToLocal(datax1[0].fecha_desde),
          end: parseAbsoluteToLocal(datax1[0].fecha_hasta)
        });
        ...
        // Assign default values if datax1 is empty
        setValor(datax1[0].valor)
        setAmortizar(datax1[0].amortizar)
        //settcontrato(0);
       //settanticipo(0);
        //settamortizado(0);
        //settamortizadosalod(0);
 ...
 setTotal(formattedValue);
  ...
  const handleGuardarInformex = async () => {
    try {
      ...
      formdata.append('image',  factura)
      try {        
        fetch(_servidorapi + 'editarinformecontratox', {
          method: 'POST',
          body: formdata
        })
      } catch (err) {
        ...
        {(onClose) => (
          <>
            <ModalHeader className="flex items-center bg-blue-500 text-white p-4">
              ...
              <Button onPress={nuevoInforme} style={{ backgroundColor: '#2EC270', color: 'white', marginRight: '20px' }}>
                Nuevo Informe
              </Button>
            </ModalHeader>
              <ModalBody>
              <Table isStriped aria-label="Example static collection table">
                <TableHeader>
                  ...
                  <TableColumn style={{ backgroundColor: '#3F8E96', color: 'white' }}>Cobrar</TableColumn>
                </TableHeader>
                  <TableBody items={informes}>
                    {(item) => (
                      <TableRow key={item.id_informe}>
                          ...
                          <TableCell>
                            <Button color="primary" onPress={() => editarInforme(item.id_informe,item.estado)}>
                              Editar
                            </Button>
                            </TableCell>
                            <TableCell>
      ...
      <ModalContent>
        {(onClose) => (
          <>
            <ModalHeader className="flex items-center bg-blue-500 text-white p-4">
              <div className="flex-grow">Informes del Contrato ::</div>
              <Button onClick={nuevoInformed} style={{ backgroundColor: '#2EC270', color: 'white', marginRight: '20px' }}>
                Nuevo Informe
              </Button>
            </ModalHeader>
              <ModalBody>
...
/>
            ...
            <Button color="primary" onClick={async () => {
              ...
              <Button color="primary" onClick={async () => {
								const result = await handleGuardarInformexe();
								if (result === null) {
									console.log('VIENE OTRA COSA')
								} else {
									onOpenChangeModal9()
