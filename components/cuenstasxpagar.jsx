...
export default function cuentasxpagar() {
  ...
  useEffect(() => {
    ...
    script.onload = () => {
      if (window.P) {
        // Suscribe los eventos 'close' y 'response'
...
const formattedDate = date.format('YYYY-MM-DD');
    ...
    useEffect(() => {
      ...
      imagex.src = '/Logox.png';
        ...
        const fetchData = async () => {
          try {
            const url = _servidorapi+'cuentasxcobrarconsaldo?id_tercero=' + encodeURIComponent(idtercero);
  ...
  <TableHeader style={{ backgroundColor: '#3F8E96', color: 'white' }}>
    ...
    <TableColumn style={{ backgroundColor: '#3F8E96', color: 'white' ,textAlign: 'center' }}>ACCIONES</TableColumn>
  </TableHeader>
...
<TableCell style={{ textAlign: 'center' }}>
          ...
          <Button  onClick={() => imprimirCuenta(item.id_cuentaxcobrar)} style={{ backgroundColor: '#7189E3', color: 'white' ,marginTop: '20px' , width: '80%',margin: '5px auto',display: 'block',textAlign: 'center'}}>
             Pdf
          </Button>  
          </div>
        </TableCell>
        <TableCell style={{ textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
          <Button 
        style={{ backgroundColor: '#ace11e', color: 'white' }} 
        onClick={handleClick}
      >
        Pago En Línea
      </Button>
          <Button onClick={() => reportarCuenta(item.id_cuentaxcobrar)} style={{ backgroundColor: '#2EC270', color: 'white' }}>
            Reportar Pago
          </Button>
          </div>
        </TableCell>
